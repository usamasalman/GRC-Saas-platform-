...
- Four of the twelve requirements (REQ-07, 08, 10, 12) are contradicted by a failing check today; REQ-01 was, until the Phase 1 fixes. The BRD screen should not say Verified for them until the defects are closed. The simplest honest fix is for that screen to read its status from the build, not from a string in `systemController.ts`.
