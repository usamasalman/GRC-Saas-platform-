...
/**
 * The module catalogue, as the server has it.
 *
 * A catalogue was hardcoded here and, on any API failure, filtered and
 * displayed as though it were live -- the comment called it "retain rich seed
 * data". An operator could browse, search and filter modules that did not
 * exist. Registering a module and changing its maturity both swallowed the
 * refusal and updated local state, so the catalogue read one way for whoever
 * made the change and another way for everyone else.
 */
const GrcModuleMarketplace: React.FC = () => {
  ...
  const loadModules = useCallback(async () => {
    ...
    try {
      ...
      setError(apiError(err, 'Could not load the module catalogue.'));
      setModules([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleAddModule = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newModName.trim() || submitting) return;
    setSubmitting(true);
    setError('');
    try {
      ...
      setNotice(`Module "${newModName.trim()}" registered.`);
      setNewModName('');
      setNewModDesc('');
      setModalOpen(false);
      await loadModules();
    } catch (err) {
      setError(apiError(err, 'Could not register the module.'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleToggleMaturity = async (mod: GrcModule) => {
    ...
    try {
      ...
      setNotice(`${mod.name} is now ${nextMaturity}.`);
      await loadModules();
    } catch (err) {
      setError(apiError(err, `Could not change the maturity of ${mod.name}.`));
    }
  };
  ...
  return (
    <div style={S.page}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap', marginBottom: 18 }}>
        <div>
          <h2 style={{ margin: 0, fontSize: 20, color: 'var(--ink)' }}>GRC Module Marketplace</h2>
      ...
      <StatStrip items={[
        ...
        ['General Availability', <span style={{ color: 'var(--success)' }}>{releasedCount}</span>],
        ['Beta / Controlled', <span style={{ color: 'var(--warning)' }}>{betaCount}</span>],
      ...
      {loading ? (
        <div style={{ color: 'var(--ink-muted)', padding: 30 }}>Loading module catalog...</div>
      ) : (
            ...
            <div key={m.id} style={{ ...S.card, padding: 16, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 8 }}>
                  <span style={{ fontSize: 10, color: 'var(--info)', background: 'var(--info-bg)', padding: '2px 6px', borderRadius: 4 }}>{m.category}</span>
                  {m.maturity === 'Released' && <span style={pill('var(--success)', 'var(--success-line)')}>GA Released</span>}
                  {m.maturity === 'Beta' && <span style={pill('var(--warning)', 'var(--warning-line)')}>Beta</span>}
                  {m.maturity === 'Planned' && <span style={pill('var(--ink-muted)', 'var(--line)')}>Planned</span>}
                </div>
                <h3 style={{ margin: '0 0 6px', fontSize: 15, color: 'var(--ink)' }}>{m.name}</h3>
                <p style={{ margin: '0 0 12px', fontSize: 12, color: 'var(--ink-muted)', lineHeight: 1.5 }}>{m.description}</p>
              </div>

              <div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 }}>
                  <span style={{ fontSize: 11, background: 'var(--surface-sunk)', color: 'var(--ink-body)', padding: '2px 8px', borderRadius: 4 }}>
                ...
                <div style={{ display: 'flex', gap: 8, borderTop: '1px solid var(--line)', paddingTop: 10 }}>
                  ...
                  <button
                    ...
                    style={{ ...ghostBtn, fontSize: 11, padding: '5px 10px', color: 'var(--info)', borderColor: 'var(--info)' }}
                  >
                    Maturity: {m.maturity}
                  </button>
                </div>
      ...
      {modalOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ ...S.card, width: '100%', maxWidth: 480, padding: 24 }}>
            <h3 style={{ margin: '0 0 16px', fontSize: 16, color: 'var(--ink)' }}>Register New GRC Module</h3>
            <form onSubmit={handleAddModule}>
              <div style={{ marginBottom: 12 }}>
                <label style={{ display: 'block', fontSize: 12, color: 'var(--ink-muted)', marginBottom: 4 }}>Module Name</label>
                <input
                  type="text"
                  required
                  value={newModName}
              ...
              <div style={{ marginBottom: 12 }}>
                <label style={{ display: 'block', fontSize: 12, color: 'var(--ink-muted)', marginBottom: 4 }}>Commercial Model</label>
      ...
      {selectedModule && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ ...S.card, width: '100%', maxWidth: 520, padding: 24 }}>
            <h3 style={{ margin: '0 0 6px', fontSize: 16, color: 'var(--ink)' }}>{selectedModule.name}</h3>
            <div style={{ fontSize: 12, color: 'var(--info)', marginBottom: 12 }}>{selectedModule.id} · {selectedModule.category}</div>
            
            <div style={{ background: 'var(--surface)', padding: 12, borderRadius: 6, border: '1px solid var(--line)', marginBottom: 14 }}>
              <div style={{ fontSize: 12, color: 'var(--ink-body)', marginBottom: 6 }}>Description:</div>
              <div style={{ fontSize: 12, color: 'var(--ink-muted)', lineHeight: 1.5 }}>{selectedModule.description}</div>
            </div>

            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 12, color: 'var(--ink-body)', marginBottom: 6 }}>Module Parameters:</div>
              <pre style={{ background: 'var(--surface-sunk)', padding: 12, borderRadius: 6, color: 'var(--info)', fontSize: 11, overflowX: 'auto' }}>
