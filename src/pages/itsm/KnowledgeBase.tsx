  ...
  return (
    <div style={S.page}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap', marginBottom: 18 }}>
        <div>
          <h2 style={{ margin: 0, fontSize: 20, color: '#f1f5f9' }}>Knowledge base</h2>
      ...
      {loading ? (
        <div style={{ color: '#64748b', padding: 30 }}>Loading articles…</div>
      ) : visible.length === 0 ? (
          ...
          {visible.map((a) => (
            <button key={a.id} onClick={() => open(a)} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, marginBottom: 8 }}>
                <strong style={{ fontSize: 13, color: '#e2e8f0' }}>{a.title}</strong>
                <span style={pill('#93c5fd', '#1e3a8a')}>{a.category}</span>
              </div>
              <div style={{
                fontSize: 12, color: '#64748b', lineHeight: 1.6, marginBottom: 10,
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
                {a.tags.slice(0, 4).map((t) => (
                  <span key={t} style={{ ...pill('#94a3b8', '#334155'), fontSize: 10 }}>{t}</span>
                ))}
              </div>
              <div style={{ fontSize: 10, color: '#475569' }}>{a.viewCount} views</div>
            </button>
          ))}
        </div>
      )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 900, padding: 20 }}>
          <div style={{ ...S.card, width: '100%', maxWidth: 640, padding: 26, borderRadius: 12, maxHeight: '88vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, marginBottom: 8 }}>
              <h3 style={{ margin: 0, fontSize: 18, color: '#f1f5f9' }}>{reading.title}</h3>
              <button onClick={() => { setReading(null); load(); }} style={linkBtn('#94a3b8')}>✕</button>
            </div>
            ...
            <div style={{
              fontSize: 13, color: '#cbd5e1', lineHeight: 1.8, whiteSpace: 'pre-wrap',
              background: '#0b1220', border: '1px solid #16202f', borderRadius: 6, padding: 16, marginBottom: 14,
            }}>
              {reading.body}
            </div>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {reading.tags.map((t) => <span key={t} style={pill('#94a3b8', '#334155')}>{t}</span>)}
              {reading.linkedTicketTypes.map((t) => <span key={t} style={pill('#93c5fd', '#1e3a8a')}>{t}</span>)}
            </div>
          </div>
        </div>
      )}
      ...
      {showNew && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 900, padding: 20 }}>
          <div style={{ ...S.card, width: '100%', maxWidth: 560, padding: 26, borderRadius: 12, maxHeight: '88vh', overflowY: 'auto' }}>
            <h3 style={{ margin: '0 0 18px', fontSize: 17, color: '#f1f5f9' }}>New knowledge article</h3>
            ...
            <form onSubmit={submit}>
              ...
              <label style={{ display: 'block', fontSize: 12, marginBottom: 5, color: '#94a3b8' }}>Body</label>
              ...
              <label style={{ display: 'block', fontSize: 12, marginBottom: 5, color: '#94a3b8' }}>Tags (comma-separated)</label>
              ...
              <label style={{ fontSize: 12, color: '#94a3b8', display: 'flex', gap: 8, alignItems: 'center', marginBottom: 20, cursor: 'pointer' }}>
