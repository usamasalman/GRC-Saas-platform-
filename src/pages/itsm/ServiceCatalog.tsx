  ...
  return (
    <div style={S.page}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap', marginBottom: 18 }}>
        <div>
          <h2 style={{ margin: 0, fontSize: 20, color: '#f1f5f9' }}>Service catalog</h2>
      ...
      <StatStrip items={[
        ...
        ['Workflow-routed', <span style={{ color: '#fbbf24' }}>{routed}</span>],
      ...
      {loading ? (
        <div style={{ color: '#64748b', padding: 30 }}>Loading catalog…</div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(320px,1fr))', gap: 12, marginBottom: 24 }}>
          {visible.map((i) => (
            <div key={i.id} style={{ ...S.card, padding: 16, borderColor: i.workflowName ? '#78350f' : '#1e293b' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, marginBottom: 6 }}>
                <strong style={{ fontSize: 14, color: '#e2e8f0' }}>{i.name}</strong>
                <span style={pill('#93c5fd', '#1e3a8a')}>{i.category}</span>
              </div>
              <div style={{ fontSize: 12, color: '#64748b', lineHeight: 1.6, marginBottom: 12 }}>{i.description}</div>

              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', fontSize: 11, marginBottom: 10 }}>
                <span style={{ color: '#94a3b8' }}>{i.ticketType}</span>
                <span style={{ color: '#475569' }}>·</span>
                <span style={{ color: '#94a3b8' }}>{i.defaultImpact}/{i.defaultUrgency} →</span>
                <span style={{ color: PRIORITY_COLOR[i.derivedPriority] || '#cbd5e1' }}>{i.derivedPriority}</span>
              </div>

              <div style={{
                borderTop: '1px solid #1e293b', paddingTop: 10, fontSize: 11,
                color: i.workflowName ? '#fbbf24' : '#64748b',
              }}>
                {i.workflowName
      ...
      <h3 style={{ fontSize: 16, color: '#f1f5f9', margin: '0 0 6px' }}>Workflow definitions</h3>
            ...
            <div key={w.id} style={S.card}>
              <button onClick={() => setExpanded(isOpen ? null : w.id)}
                style={{
                  ...
                  color: '#e2e8f0', fontFamily: 'inherit', fontSize: 13, textAlign: 'left', gap: 12, flexWrap: 'wrap',
                }}>
              ...
              {isOpen && (
                <div style={{ borderTop: '1px solid #1e293b', padding: '14px 16px' }}>
                  {w.description && <div style={{ fontSize: 12, color: '#64748b', marginBottom: 12 }}>{w.description}</div>}
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'stretch' }}>
                    {w.steps.map((s: any, idx: number) => (
                      <React.Fragment key={s.key}>
                        <div style={{ background: '#0b1220', border: '1px solid #16202f', borderRadius: 6, padding: '8px 12px', minWidth: 150 }}>
                          <div style={{ fontSize: 12, color: '#e2e8f0' }}>{s.name}</div>
                          <div style={{ fontSize: 10, color: '#64748b', marginTop: 3 }}>{s.type}</div>
                          {s.requiredCapability && (
                            <div style={{ fontSize: 10, color: '#93c5fd', marginTop: 3 }}>needs: {s.requiredCapability}</div>
                          )}
                          {s.sodGuardedAction && (
                            <div style={{ fontSize: 10, color: '#fca5a5', marginTop: 2 }}>SoD: {s.sodGuardedAction}</div>
                          )}
                          {s.dueInHours && (
                            <div style={{ fontSize: 10, color: '#475569', marginTop: 2 }}>due in {s.dueInHours}h</div>
                          )}
                        </div>
                        {idx < w.steps.length - 1 && (
                          <div style={{ display: 'flex', alignItems: 'center', color: '#334155' }}>→</div>
                        )}
                      </React.Fragment>
                    ))}
                  </div>
